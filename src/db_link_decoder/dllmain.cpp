...
static bool good;
	...
	#define DB_LINK_DECODER_CONFIG_FILE_NAME "..\\conf\\db_link_decoder.ini"
#else
	#define DB_LINK_DECODER_CONFIG_FILE_NAME "../conf/db_link_decoder.ini"
#endif
#define DB_LINK_DECODER_CONFIG_FILE_NAME2 "db_link_decoder.ini"

bool init_decoder(const std::map<std::string, int>& cdrex_index) {
	good = false;
	config conf;
	conf.open(DB_LINK_DECODER_CONFIG_FILE_NAME);
	if( !conf.good() ) {
		conf.open(DB_LINK_DECODER_CONFIG_FILE_NAME2);
		if( conf.good() ) {
			return good;
		}
	}
...
bool create_decoder(const std::string& name, decoder_ptr& decoder) {
	...
	if( it != definitions.end() ) {
		decoder.reset(new db_link_decoder(&(*it).second, name));
		return true;
	}
	return false;
}
